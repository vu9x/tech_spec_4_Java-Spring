...
public class IssueController {
    ...
    public ResponseEntity<Issue> issueBook(@RequestBody IssueRequest issueRequest){
        ...
        try{
//            meterRegistry.counter("issued_books").increment();
//            Counter.builder("issued_books").description("Количество выданных книг").register(meterRegistry).increment();
            return ResponseEntity.status(HttpStatus.CREATED).body(service.createIssue(issueRequest)) ;
        } catch(NoSuchElementException e) {
//            meterRegistry.counter("issued_books_failed").increment();
//            Counter.builder("Failed_to_issue_books").description("Количество ошибок при выдаче книг").register(meterRegistry).increment();
